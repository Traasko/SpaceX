...
const Quiz = (answer) => {
        ...
        const [trueAnswer, setTrueAnswer] = useState(questionsList[questionState-1]["answer"]);
                ...
                return (
                    <>
                        {options.map((option, index) => (
                            <>
                                <button  key={index} id={index} onClick={scoring} value={option} className={props.selectedOption === index ? 'selected' : ''}>{option}</button>
                            </>
                        ))}
                    </>
                )
